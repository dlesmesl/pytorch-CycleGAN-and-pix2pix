...
class HybridModel(BaseModel):
    ...
    def backward_G(self, mode='aligned'):
        ...
        self.loss_idt_A = self.criterionIdt(
            self.idt_A, self.real_B) * lambda_idt
        # G_B should be identity if real_A is fed: ||G_B(A) - A||
        self.idt_B = self.netG_B(self.real_A)
        self.loss_idt_B = self.criterionIdt(
            self.idt_B, self.real_A) * lambda_idt
        ...
        # Full loss
        # self.loss_G_total = (self.loss_G_GAN * 1) + (self.loss_cycle * lambda_A) + (self.loss_G_idt * lambda_idt)
        self.loss_G_total = self.loss_G_GAN + self.loss_cycle + self.loss_G_idt
        if mode == 'aligned':
            self.loss_G_total += (self.loss_G_L1 * lambda_L1)
